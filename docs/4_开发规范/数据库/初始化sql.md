本文档约 52000 字，阅读本文档约 100 分钟。

# 自动化安全评估系统数据库初始化 SQL

## 1. 使用说明

本文档给出自动化安全评估系统 MVP 的 PostgreSQL 基线结构。SQL 面向全新数据库，一次执行完成 16 张表、约束、索引、注释和触发器的创建。后续结构变更必须通过 Alembic migration 管理，不允许应用启动时隐式改表。

初始化脚本不创建管理员、不写入运行配置，也不保存密码明文、私有仓库凭证、Token、证书或其他密钥。管理员必须通过 `POST /api/v1/system/init` 创建，运行参数必须由管理员确认后通过配置接口写入。

## 2. 兼容性与执行前提

- 目标数据库：PostgreSQL 15 及以上。
- 必需扩展：`pgcrypto`，用于 `gen_random_uuid()`。
- 执行账户需要在目标 schema 中创建扩展、函数、表、索引和触发器的权限。
- 应在空数据库或空 `public` schema 中执行。脚本除扩展外不使用 `IF NOT EXISTS`，结构冲突时应失败并回滚。
- 数据库会话时区建议设置为 `UTC`；所有时间列均使用 `timestamptz`。
...
验证结果中目标表数量必须为 `16`。`attack_path_items` 应存在延迟约束触发器，9 张可变业务表应存在 `updated_at` 触发器。

## 5. 与 Alembic 的衔接

- 本文 SQL 是 MVP 基线结构的权威设计表达，不应与 Alembic 首个 revision 各自独立维护。
- 项目落地时应把同一结构转换为首个 Alembic revision，并在测试数据库执行 `alembic upgrade head` 后复用本节验证查询。
- 生产环境只使用 Alembic migration 演进结构，不再次执行整份初始化 SQL。
- 状态值、约束或索引变更必须单独创建 migration，升级和降级路径均需评审。
- PostgreSQL `CHECK`、复合外键、部分索引、延迟唯一约束和约束触发器必须在 migration 中显式保留，不能只存在于 SQLAlchemy Model。

## 6. 注意事项

- `projects.source_path` 在 `local` 模式下保存管理员授权根目录内的相对路径；在 `repository` 模式下保存已移除内联密码的仓库地址。
- `project_runtimes.project_id` 唯一，数据库层落实 MVP 每项目最多一个运行实例。未来支持重跑时必须通过 migration 调整该约束。
- `domain_events.sequence` 由领域服务在项目级事务锁内分配。推荐在事务中取得项目行锁或项目级 advisory lock，再写业务状态和 Outbox 事件；唯一约束负责兜底拒绝冲突。
- `attack_path_items.step_order` 在事务提交时检查从 `1` 连续。批量重排必须在同一事务中完成。
- `reports.report_file_path` 和 `file_artifacts.logical_key` 都是逻辑键。报告进入 `ready` 前，应在同一业务事务中写入报告记录与对应报告制品元数据。
- 数据库级联只删除文件元数据，不能删除容器、宿主机文件或对象存储对象。项目清理任务必须先完成外部资源清理，再删除项目记录。
- `audit_logs.metadata`、`domain_events.payload` 和 `project_runtimes.environment_snapshot` 写入前必须执行字段白名单和敏感信息过滤。
